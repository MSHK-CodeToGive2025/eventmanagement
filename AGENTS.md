...
- **WhatsApp (no freeform)**: All manual WhatsApp sends use the **event update template** (`TWILIO_WHATSAPP_EVENT_UPDATE_TEMPLATE_SID`, utility). Variables: {{1}} Event, {{2}} Session, {{3}} Message, {{4}} Contact, {{5}} Phone. Freeform (body-only) messages are not supported and have been removed to avoid error 63016.
