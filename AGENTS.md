...
- **WhatsApp (no freeform)**: All manual WhatsApp sends use the **event update utility template** (`TWILIO_WHATSAPP_UPDATE_TEMPLATE_SID`). Variables: 1 = event title, 2 = session, 3 = message body, 4 = contact name, 5 = contact phone. Freeform (body-only) messages are not supported and have been removed to avoid error 63016.
