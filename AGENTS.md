...
- **WhatsApp 24-hour session window**: Freeform/custom WhatsApp messages only deliver if the recipient messaged the business within the last 24 hours. Outside this window, Twilio accepts the API call (HTTP 201) but fails delivery asynchronously with error 63016. The bulk send route (`/:id/send-whatsapp`) polls message status after sending and falls back to a marketing template (`TWILIO_WHATSAPP_MARKETING_TEMPLATE_SID`) if 63016 is detected. Marketing templates may also be rate-limited by Meta (error 63049).
