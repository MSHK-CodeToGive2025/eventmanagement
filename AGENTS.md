...
- **WhatsApp (no freeform)**: WhatsApp sends use **event update templates** (`TWILIO_WHATSAPP_EVENT_UPDATE_SINGLE_SESSION_TEMPLATE_SID` / `TWILIO_WHATSAPP_EVENT_UPDATE_MULTIPLE_SESSION_TEMPLATE_SID` / `TWILIO_WHATSAPP_UPDATE_TEMPLATE_SID`). Multiple sessions variables: 1=firstName, 2=event title, 3=session, 4=message body, 5=contact name, 6=contact phone. Single session variables: 1=firstName, 2=event title, 4=message body, 5=contact name, 6=contact phone ({{3}} omitted).
- **Event reminders**: Sent by the reminder service using reminder templates (`TWILIO_WHATSAPP_EVENT_REMINDER_SINGLE_SESSION_TEMPLATE_SID` [9 vars: 1, 2, 4..10] / `TWILIO_WHATSAPP_EVENT_REMINDER_MULTIPLE_SESSION_TEMPLATE_SID` [10 vars: 1..10]) or event update templates. The cron runs **every 5 minutes** in Asia/Hong_Kong.
