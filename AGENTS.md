...
- **Event reminders**: Sent by the reminder service using either the 8-variable reminder template (`TWILIO_WHATSAPP_TEMPLATE_SID` / `zubin_foundation_event_reminder_v2`) or the event update template (`TWILIO_WHATSAPP_UPDATE_TEMPLATE_SID` / `zubin_foundation_event_update_v3`). The cron runs **every 5 minutes** in Asia/Hong_Kong. In production the backend must run continuously (e.g. ECS task) so the cron runs; ensure `TWILIO_WHATSAPP_TEMPLATE_SID` and `TWILIO_WHATSAPP_UPDATE_TEMPLATE_SID` are set in the backend environment.
