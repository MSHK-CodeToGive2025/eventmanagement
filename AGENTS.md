...
- **WhatsApp (no freeform)**: All manual WhatsApp sends use the **event update (notification) template** (`TWILIO_WHATSAPP_NOTIFICATION_TEMPLATE_SID`, `zubin_foundation_event_update_v2`). 5 variables: {{1}} event title, {{2}} session, {{3}} message body, {{4}} contact name, {{5}} contact phone. Freeform (body-only) messages are not supported and have been removed to avoid error 63016.
- **Event reminders**: Sent by the reminder service using either the 8-variable reminder template (`TWILIO_WHATSAPP_TEMPLATE_SID`, `zubin_foundation_event_reminder_v2`) or the 5-variable notification template (`TWILIO_WHATSAPP_NOTIFICATION_TEMPLATE_SID`). The cron runs **every 5 minutes** in Asia/Hong_Kong. In production the backend must run continuously (e.g. ECS task) so the cron runs; ensure `TWILIO_WHATSAPP_TEMPLATE_SID` and `TWILIO_WHATSAPP_NOTIFICATION_TEMPLATE_SID` are set in the backend environment.
